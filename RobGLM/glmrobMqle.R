...
glmrobMqle <-
           ...
           trace = FALSE)
  {
    ...
    ncoef <- ncol(X)
    if(is.numeric(weights.on.x)){ 
      if(length(weights.on.x) == nobs & !(any(weights.on.x < 0)) & !(any(weights.on.x > 1))){
        w.x <- weights.on.x
      }else{
        stop("All weights.on.x must be non-negative and smaller or equal to 1")
      }
    }else{
